...
int main(int argc, char* argv[]){
	...
	while ((opt = getopt(argc, argv, "t:")) != -1) {
		switch (opt) {
		case 't':
			strncpy(remote_address, optarg, 18);
			printf("> target address: %s\n", remote_address);
			break;
		default:
			printf("> usage:  -t <target BDADDR>\n");
			return 0;
		}
	}
	...
	fprintf(stdout, "<SPP>: SPP Connection success!\n");

	artik_bt_gatt_char_properties prop1;
	ret = bt->gatt_get_char_properties(remote_address, SPP_SERVICE, SPP_DATA_CHAR, &prop1);
	if (ret != S_OK){
		fprintf(stdout, "<ERROR>: Gather properties error!\n");
		goto spp_quit;
	}
	fprintf(stdout, "<SPP>: Gahter data OK!\n");

	ret = bt->gatt_start_notify(remote_address, SPP_SERVICE, SPP_DATA_CHAR);
	if (ret != S_OK){
		fprintf(stdout, "<ERROR>: Start notify ERROR!\n");
        goto spp_quit;
        }
	...
	while(image_pointer < Image_Size){
		...
		bt->gatt_char_write_value(remote_address, SPP_SERVICE, SPP_DATA_CHAR, byte, tam_paq);
